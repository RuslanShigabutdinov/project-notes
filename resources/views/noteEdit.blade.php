...
@section('title', 'Project Notes - Note Edit')
...
@section('content')
    ...
    <div class="card" 
    style="background: repeating-linear-gradient(#0000 0 calc(1.2rem - 1px),{{ $note->line_color }} 0 1.2rem) 
    right bottom /100% 100%,linear-gradient({{ $note->note_color }} 0 0) 30px 0/2px 100% {{ $note->note_color }}">
        <form action="/note/update" method="post" enctype="multipart/form-data" id="noteForm">
            @csrf
            ...
            <div class="card_content">
                <input type="text" class="inputTitle" value="{{ $note->title }}" style="background: {{ $note->note_color }}; color: {{ $note->text_color }}">
                <div id="editor">{!! $note->content !!}
                </div>
    ...
    <div id="color-modal" class="modal">
        <div class="modal__content">
            <form action="/note/color/update" method="post" id="colorForm">
                @csrf
                <h2>Цвет заметки</h2>
                <input type="hidden" name="id" value="{{ $note->id }}">
...
@section('jsScripts')
    ...
    <script>
        ...
        let color1 = Pickr.create({
            el: colorPicker1,
            useAsButton: true,
            theme: 'classic',
            default: '{{ $note->note_color }}',
            closeWithKey: 'Escape',
            swatches: [
                'rgba(244, 67, 54, 1)',
                'rgba(233, 30, 99, 0.95)',
        ...
        let color2 = Pickr.create({
            el: colorPicker2,
            useAsButton: true,
            theme: 'classic',
            default: '{{ $note->text_color }}',
            closeWithKey: 'Escape',
            swatches: [
                'rgba(244, 67, 54, 1)',
                'rgba(233, 30, 99, 0.95)',
        ...
        let color3 = Pickr.create({
            el: colorPicker3,
            useAsButton: true,
            theme: 'classic',
            default: '{{ $note->line_color }}',
            closeWithKey: 'Escape',
            swatches: [
                'rgba(244, 67, 54, 1)',
                'rgba(233, 30, 99, 0.95)',
